...
export default function LeaseAccountingError({
  error,
  reset,
}: {
  error: Error;
  ...
  React.useEffect(() => {
    console.error('Lease accounting route error:', error);
  }, [error]);

  return (
    <div className="p-6 border border-destructive/20 rounded-lg bg-destructive/5 text-center space-y-4 max-w-lg mx-auto my-12">
      <h3 className="text-lg font-bold text-destructive">Failed to load lease ledger summary</h3>
