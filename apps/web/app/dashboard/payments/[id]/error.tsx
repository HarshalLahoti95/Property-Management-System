...
export default function PaymentDetailError({
  ...
  React.useEffect(() => {
    console.error('Payment detail route error:', error);
  }, [error]);

  return (
    <div className="p-6 border border-destructive/20 rounded-lg bg-destructive/5 text-center space-y-4 max-w-lg mx-auto my-12">
      <h3 className="text-lg font-bold text-destructive">Failed to load payment detail</h3>
      ...
      <div className="flex justify-center gap-4">
        ...
        <Button onClick={() => (window.location.href = '/dashboard/payments')} variant="outline">
          Return Dashboard
        </Button>
      </div>
    </div>
